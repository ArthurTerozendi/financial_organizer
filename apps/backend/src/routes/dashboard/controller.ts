...
export async function getTransactionsGroupByMonth(request: FastifyRequest, reply: FastifyReply) {
  try {
    const { id } = request.user;

    const lastTwelveMonths = getLastTwelveMonths();

    const transactions = await Db.instance.$queryRaw<{ yearMonth: string, creditAmount: number, debitAmount: number }[]>`
      SELECT
    ...
    `;
     ...
     const transactionsMap = new Map<string, { creditAmount: number; debitAmount: number }>();
    transactions.forEach(t => transactionsMap.set(t.yearMonth, {
